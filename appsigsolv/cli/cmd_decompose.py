...
def run_decompose(args):
    ...
    if args.component.lower() == "all":
        ...
        raw_cols = [c for c in df_peek.columns if c != detected_date_col]
        components_to_process = []
        for c in raw_cols:
            try:
                components_to_process.append(f"{round(float(c), 3):g}")
            except ValueError:
                components_to_process.append(c)
    ...
    for comp in components_to_process:
        ...
        if json_path.exists() and not getattr(args, "force", False):
            print(f"  [info] Component '{comp}' already processed. Skipping. (use --force to overwrite)")
            continue
        ...
        try:
            ...
            final_periods = [float(p) for p in final_periods]
            final_periods.sort()
            ...
            print(f"  [error] Component '{comp}' failed during fitting/output: {e}")
            import traceback
            traceback.print_exc()
            continue
